...
async function readBearer() {
    if (IS_TEST_DB)
        return SUPABASE_ANON_KEY; // the test twin is anon-open
    try {
...
export async function loadTodos() {
    ...
    const res = await fetch(`${TABLE_URL}?select=*&tag=eq.${encodeURIComponent(TODO_TAG)}&order=created_at.asc`, { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${bearer}` } });
    if (!res.ok)
 ...
 */
export async function addTodo(content, source) {
