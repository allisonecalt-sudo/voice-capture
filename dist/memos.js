 ...
 */
async function insertMemo(content, tag, source) {
    const payload = { content, source };
 ...
 */
async function setMemoStatus(id, status) {
    const bearer = await readBearer();
    if (!bearer)
        throw new Error('not logged in');
    const body = status === 'done' ? { status, done_at: new Date().toISOString() } : { status, done_at: null };
...
async function beginRecording() {
    if (recorder)
        return; // double-tap guard — same hole app.ts v34 closed
    if (!geminiKey()) {
...
// ── Actions ─────────────────────────────────────────────────────────────────
async function addMemo(content, source) {
...
async function toggleMemo(id, to) {
    if (state.busyIds.has(id))
        return;
...
function groupedActive() {
    ...
    if (!filter && untagged.length)
        groups.push({ label: GENERAL_GROUP, memos: untagged });
    return groups;
}
...
function renderChips() {
    ...
    const chips = tags
        .map((t) => `<button type="button" class="chip${state.filterTag === t ? ' is-active' : ''}"
...
function renderList() {
    ...
    const groupHtml = groups
      ...
      <section class="memo-group">
        ...
        .join('');
    const emptyFiltered = !groups.length && done ? `<p class="memos-status">Nothing active — all checked off. ✓</p>` : '';
...
function wire() {
    ...
    if (mic)
        mic.addEventListener('click', () => void beginRecording());
