...
export const GEMINI_MODELS = [GEMINI_MODEL, 'gemini-flash-latest'];
...
export async function transcribeAudio(apiKey, base64, mimeType) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        if (RETRYABLE.has(res.status)) {
            throw new Error("Gemini is busy right now (Google's servers, not your key). Your recording is " +
